...
```{r internal-functions, cache = TRUE}
...
```{r format-size, include=FALSE, cache = TRUE}
...
packages_updates |> 
  group_by(month = floor_date(d, "month")) |> 
  ...
  mutate(type = case_when(
    ...
    TRUE ~ "weird")) |> 
  ggplot() +
  geom_smooth(aes(month, p, 
                  col = fct_relevel(type, c("Quick & frequent", "Quick", "Frequent", "Ideal"))), span = "1 year") +
  theme_minimal() +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
...
Usually 80% of the packages submitted each day follow the policies without troubles.
Around 40%
...
If we look at the time since the previous release we might get some clues:
...
We see that currently the second update is sooner than previously but after the second update the time between releases is similar and higher than the first update. (Note that here we didn't remove the first weeks before a package is established on CRAN.).
This has been happening since 2007 with a similar gap between the first and second update (second and third release). 

However, since 2013 there is an increase in time till the next release. 
...
dates_approval |> 
  ...
  geom_textsmooth(aes(month, n, 
                  col = fct_other(uname, keep = people, other_level = "other"),
              linetype = fct_other(uname, keep = people, other_level = "other"),
              label = fct_other(uname, keep = people, other_level = "other"))) +
  labs(col = "Users",
       linetype = "Users",
       y = "Moves",
...
```{r setup-dependencies, cache = TRUE}
...
Compare the archives records with MRAN via [checkpoint](https://cran.r-project.org/package=checkpoint). Perhaps also to provide further data about packages before the start of MRAN (2014-09-17).
This would imply detecting when packages were archived on CRAN. 

Provide reverse dependency along time as done with [revdepcheck.extras](https://github.com/HenrikBengtsson/revdepcheck.extras/blob/master/R/revdep_over_time.R). This would mean recreating a file with the right structure for `tools::packages_dependencies()` for each date. 
