...
The monthly tally of packages shows some waves with less packages on CRAN accepted late in the year.

If we look at the accumulated packages on CRAN that still remain on CRAN (packages might have been accepted but later removed), we see an exponential growth:
...
This doesn't mean that the packages first time accepted on CRAN didn't had previous submissions and builts, but when they finally built and pass the checks they are handled as with the other packages already on CRAN.
...
Althought Zileis also signs the R-journal sections about CRAN. 
...
The number of packages currently on CRAN `r nrow(current)` and those currently not on CRAN is very different.
...
Also note that in 2021 there was the first reduction in packages approvals. 
...
It is also worth mentioning that big increase in packages approved for inclusion on CRAN in 2020 (effect of the COVID pandemic?).
...
This has remained more or less stable with time close to 90%. 
...
dates_approval |> 
  ...
  geom_line(aes(year, n, 
                col = forcats::fct_other(uname, keep = people, other_level = "other"))) +
  labs(col = "Users") +
  theme_minimal() +
...
dates_approval |> 
  ...
  ggplot() +
  geom_smooth(aes(month, n, col = forcats::fct_other(uname, keep = people, other_level = "other"))) +
  labs(col = "Users")
```
...
dates_approval |>  
  ...
  ggplot() +
  geom_smooth(aes(week, n, col = forcats::fct_other(uname, keep = people, other_level = "other"))) +
  labs(col = "Users")
...
dates_approval |>  
  ...
  ggplot() +
  geom_smooth(aes(wday, n,  col = forcats::fct_other(uname, keep = people, other_level = "other")), method = "glm") + 
  labs(col = "Users")
```
...
dates_approval |> 
  ...
  ggplot() +
  geom_smooth(aes(mday, n, col = forcats::fct_other(uname, keep = people, other_level = "other"))) +
  labs(col = "Users") +
  coord_cartesian(ylim = c(0, NA)) +
  scale_x_continuous(expand = expansion(), breaks = dmonth_breaks,
                       position = "top") +
...
dates_approval |> 
  ...
  ggplot() +
  geom_smooth(aes(yday, n, col = forcats::fct_other(uname, keep = people, other_level = "other"))) + 
  labs(col = "Users") +
  annotate(geom = "text", x = 100, y = 12, label = "April") +
  annotate(geom = "text", x = 200, y = 12, label = "July") +
  annotate(geom = "text", x = 300, y = 12, label = "November") +
