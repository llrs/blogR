...
So if you want to have all the captions below the element you'll need to use a different formatter for it (booktable, or others).
...
This code basically means that I need to add `\ChapFrame` when I want the chapter thumb (I didn't know the namb before searching this).
...
A tiny trick I learned was to reset the counter with `\afterpage{\setcounter{chapshift}{0}}` after the biblography so that the appendix would use the same mark from the beginning.
...
To add them I used the tocbibind package that automatically adds it (wasn't sure from the [answer](https://tex.stackexchange.com/a/48512/178206) I found online).
...
Following an [answer](From%20https://stackoverflow.com/a/33801326/2886003), the figure floating algorithm was set with these preferences:
...
`\chapFrame` is the new command defined to set [chapter thumb](#Chatper-thumb).
