...
[^1]: the `maintainer` function only works for installed packages, and there are
...
Both logs only count those of their repository and not from other mirrors or approaches (RSPM, bspm, r2u, ....).
...
There is an outlier on Fig1, the mlr package has more than 10k downloads and close to 120 direct dependencies!
...
Most maintainer have few packages but some have many 
...
ggplot(pca_data) +
  ...
  geom_text_repel(aes(PC1, PC2, label = package), data = . %>% filter(PC1 > 5)) +
  theme_minimal() +
...
summary(pca_strong)
pca_data_srong <- cbind(pca_strong$x, deps_wo[deps_wo$strong, ])
ggplot(pca_data_srong) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_point(aes(PC1, PC2, col = repository, shape = repository)) +
...
ggplot(pca_data_weak) +
  ...
  theme_minimal() +
  theme(axis.text = element_blank()) +
  labs(title = "Packages suggesting XML and RCurl", 
       subtitle = "PCA of numeric variables of weak dependencies",
       col = "Repository", shape = "Repository")
```
...
-   Evaluate if the xml2 and curl packages provides enough functionality to replace XML and RCurl respectively. If not see what should be added to these packages or how to develop alternative packages to fill the gap.
-   Contact package maintainers to replace the functionality they currently depend on XML and RCurl.
