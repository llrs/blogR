...
Now that we added the references to the files we can check some 

```{r}

```

Now we can add the links that use the file names instead of topics.
...
xrefBase4_m <- merge(missing, all_aliases[, -2], 
            by.x = c("to_pkg", "to_target"), 
...
We added a column to add this information. 
We need to do the same with CRAN links

```{r}
missing <- rdxrefsCRAN3 |> 
  filter(is.na(Rd_destiny), !is.na(to_pkg), !is.na(to_target)) |> 
  select(-Rd_destiny, Source = Rd_origin) |> 
  mutate(n = seq_len(n()))

rdxrefsCRAN3_m <- merge(missing, all_aliases[, -2], 
            by.x = c("to_pkg", "to_target"), 
            by.y = c("Package", "filename"), 
            all.x = TRUE, sort = FALSE) |> 
...
We can continue adding some more columns to the data for different conditions:
...
count(m, CRAN_checks, missing_anchors) 
m |> filter(is.na(CRAN_checks)) |> 
  count(missing_anchors, 
linking2base, linking2cran, linking2bioc, missing_dependency, , 
missing_package, sort = TRUE) |> 
  arrange(n)
