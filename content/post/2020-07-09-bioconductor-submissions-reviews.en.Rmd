...
Almost all the submitted packages without a reviewer (`r sum(by_issue$assignments == 0)` submissions) were rejected except for three: `r link_issue(81:83)` that were reviewed by `r link_profile("vobencha")` even if not officially assigned.
...
revi %>% 
  ...
  facet_wrap(~n_reviewers) +
  labs(x = element_blank(), y = "Comments", title = "Comments from reviewers",
...
cm %>% 
  ...
  mutate(perc = n/sum(n)) %>% 
  ggplot() +
  geom_col(aes(id, perc, fill = fct_lump_n(reason, 7), 
               col = fct_lump_n(reason, 7),
               group = reason),
           width = 1) +
  labs(fill = "Type of comment", col = "Type of comment",
...
We can see clearly differences on the behavior of the issues, some comments are all of the non-accepted packages. 
...
```{r common_feedback}
build_related <- c("Build result", "Building", "Valid push", "Received", "Update version")

build_bot <- bioc_bot %>% 
  group_by(id) %>% 
  summarise(builds = sum(reason %in% build_related), total = n(), 
            Approved = unique(Approved))
...
bioc_bot %>% 
  group_by(id) %>% 
  summarise(Approved = unique(Approved),
            Received = sum(reason == "Received"),
            Valid = sum(reason == "Valid push"),
            update = sum(reason == "Update version"),
             ) %>% 
  ggplot() +
  geom_count(aes(Received, Valid, col = update)) +
  facet_wrap(~Approved, scales = "free") +
  scale_radius() +
  labs(x = "Received push", y = "Valid push", size = "Issues", col = "Update")
```
