...
Overall, `r lp(sum(fail)/sum(all_subm$total_attempts))` of the attempts end up being rejected.

## Main take away

Submitting to CRAN is not easy on the first try. 
While [Writing R extensions](https://cran.r-project.org/doc/manuals/r-devel/R-exts.html) document is clear, it is too extensive for many cases.
The [CRAN policy](https://cran.r-project.org/web/packages/policies.html) is short but it is not clear enough for new maintainers.
In additions, it seems that packages having more problems with the submissions are not those new: experienced maintainers have troubles getting their pacakge accepted when submitting them.
