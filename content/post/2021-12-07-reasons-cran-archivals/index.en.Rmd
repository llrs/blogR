...
The file is incomplete as it doesn't have some archival records of some packages, but there are old records from 2013 to now so we can use it to see understand what are the reasons of removal of packages.

# The data

First step is read the record. 
As it seems that it has some `key: value` structure similar to DESCRIPTION file of packages it seems it is a DCF format: Debian Control File format.
...
Now that it is in R data structure, we can extract the relevant information, dates, type of action and reasons for each archival event.
...
history |>  
  ...
  ggplot() +
  geom_col(aes(date, n, fill = action)) +
  labs(y = "Events", fill = "Action") +
  theme_minimal() +
...
I know that there are some old events not recorded on the file, because there are some packages currently present on CRAN that they have an archived event but not an unarchived one.
So, this doesn't necessarily mean that there are currently more packages archived from CRAN. 
...
hist_expand <- history |> 
  ...
  mutate(not_corrected = grepl("not correct", reason, fixed = TRUE),
         ...
         request_maintainer =  maintainer & request,
...
We can look at the most frequent error reasons for archiving packages:
...
The third most common reason seems to be at the request of the maintainer.   
...
hist_summary <- hist_expand |> 
  ...
  mutate(package_not_corrected =  not_corrected | check | reminders | install | update,
         ...
         other = email_error | licence | authorship | unresponsive_maintainer|  request_author) |> 
